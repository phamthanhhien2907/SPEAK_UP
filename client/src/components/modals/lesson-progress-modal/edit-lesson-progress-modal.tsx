...
import { apiUpdateLessonProgress } from "@/services/lesson-progress.services";
...
export const EditLessonProgressModal = () => {
  const [lessonData, setLessonData] = useState([]);
  const [userData, setUserData] = useState([]);
  const { isOpen, onClose, type, data } = useModal();
  const { lessonProgress } = data;
  const isModalOpen = isOpen && type === "editLessonProgress";
  const form = useForm({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    console.log(values);
    const res = await apiUpdateLessonProgress(lessonProgress?._id, {
      ...values,
  ...
  useEffect(() => {
    if (lessonProgress) {
      form.setValue("lessonId", lessonProgress.lessonId?._id);
      form.setValue("userId", lessonProgress.userId?._id);
      form.setValue("score", lessonProgress.score);
    }
  }, [form, lessonProgress]);
  useEffect(() => {
