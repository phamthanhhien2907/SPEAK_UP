...
const seed = async () => {
    ...
    const fetchVocabulary = async (wordPairs: { correct: string; wrong: string }[], sound: string) => {
        ...
        for (const pair of wordPairs) {
            ...
            try {
                ...
                const audioUrl = data.phonetics?.[0]?.audio || `https://example.com/audio/${pair.correct}.mp3`;
                ...
                if (meanings.length > 0) {
                    ...
                    if (!existingWords.has(pair.correct)) {
                        vocabItems.push({
                            word: pair.correct,
                            phonetic,
                            meaning,
                        ...
                        existingWords.add(pair.correct);
                    }
                }
            } catch (error: any) {
            ...
            try {
                ...
                const data = response.data[0];
                const phonetic = data.phonetic || data.phonetics?.[0]?.text || "";
                const audioUrl = data.phonetics?.[0]?.audio || `https://example.com/audio/${pair.wrong}.mp3`;
                const meanings = data.meanings || [];
                if (meanings.length > 0) {
                    const meaning = meanings[0].definitions[0].definition || "No definition available";
                    const example = meanings[0].definitions[0].example || `${pair.wrong} in a sentence.`;
                    if (!existingWords.has(pair.wrong)) {
                        vocabItems.push({
                            word: pair.wrong,
                            phonetic,
                            meaning,
